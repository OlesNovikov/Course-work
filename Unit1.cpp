...
int ReadFromFile(F *field, char *path) {
        int count = 0;
        FILE* f = fopen(path, "r");
        while (!(feof(f))){
...
void ViewInChat(int LeftCoub, int RightCoub) {
        const int jail = 10, start = 0, gotojail = 30, jackpot = 20;
        AnsiString str;
        ...
        if ((field[pl -> CurrentField].cost != 0) && (pl -> CurrentField != 4) && (pl -> CurrentField != 36)) {
                if ((field[pl -> CurrentField].owner != 0) && (field[pl -> CurrentField].owner != pl -> color)) {
                        Monopoly -> Chat -> Lines -> Add(AnsiString(pl -> name) + " попадает на " + str + " и должен заплатить аренду " + AnsiString(field[pl -> CurrentField].pay) + "k");
                } else if ((field[pl -> CurrentField].owner != 0) && (field[pl -> CurrentField].owner == pl -> color)) {
                        Monopoly -> Chat -> Lines -> Add(AnsiString(pl -> name) + " попадает на своё поле и ничего не платит");
                } else {
                        Monopoly -> Chat -> Lines -> Add(AnsiString(pl -> name) + " попадает на " + str + " и задумывается о покупке");
                }
        } else if ((pl -> CurrentField == 4) || (pl -> CurrentField == 36)) {
                Monopoly -> Chat -> Lines -> Add(AnsiString(pl -> name) + " попадает на Налог и должен заплатить " + AnsiString(field[pl -> CurrentField].cost) + "k");
        } else if ((pl -> CurrentField == 2) || (pl -> CurrentField == 7) || (pl -> CurrentField == 17) || (pl -> CurrentField == 22) || (pl -> CurrentField == 33) || (pl -> CurrentField == 38)) {
                Monopoly -> Chat -> Lines -> Add(AnsiString(pl -> name) + " попадает на поле " + str);
        } else if (pl -> CurrentField == jail) {
                Monopoly -> Chat -> Lines -> Add(AnsiString(pl -> name) + " попадает на экскурсию в тюрьму");
        } else if (pl -> CurrentField == gotojail) {
                Monopoly -> Chat -> Lines -> Add(AnsiString(pl -> name) + " отправляется в тюрьму за мошенничество");
        } else if (pl -> CurrentField == jackpot) {
                Monopoly -> Chat -> Lines -> Add(AnsiString(pl -> name) + " попадает на Джекпот и может попытать удачу ");
        } else {
...
void __fastcall TMonopoly::FormCreate(TObject *Sender)
{
        ...
        int count = ReadFromFile(field, path);
        HideButtons();
        HideFields();
}
//---------------------------------------------------------------------------
...
//==============================================================================
        if ((field[pl -> CurrentField].cost != 0) && (pl -> CurrentField != 4) && (pl -> CurrentField != 36)) {
                if ((field[pl -> CurrentField].owner != 0) && (field[pl -> CurrentField].owner != pl -> color)) {
                        if ((pl -> CurrentField == 12) || (pl -> CurrentField == 28)) field[pl -> CurrentField].pay *= pl -> CoubSum;
                        BitBtn3 -> Caption = "Заплатить " + AnsiString(field[pl -> CurrentField].pay * pl -> CoubSum) + "k";
...
void __fastcall TMonopoly::Timer1Timer(TObject *Sender)
{
        ...
        Panel16 -> Hide();
        GroupBox1 -> Show();
        if (field[pl -> CurrentField].cost != 0) {
                GroupBox2 -> Show();
        }
}
//---------------------------------------------------------------------------
void __fastcall TMonopoly::BitBtn3Click(TObject *Sender)
{
        ...
        Chat -> Lines -> Add(AnsiString(pl -> name) + " покупает " + str + " за " + AnsiString(field[pl -> CurrentField].cost) + "k");
                ...
                pl -> cash -= field[pl -> CurrentField].cost;
                Chat -> Lines -> Add(AnsiString(pl -> name) + " заплатил 1000k банку ");
        }
                switch (pl -> color) {
                        case 1: Player1Money -> Caption = "$" + IntToStr(pl -> cash) + "k";
                                break;
